...
import { ptyPath, conPtyPath } from './prebuild-file-path';
...
export class WindowsPtyAgent {
  ...
  constructor(
    ...
    if (this._useConpty) {
      if (!conptyNative) {
        try {
          conptyNative = require(conPtyPath || '../build/Release/conpty.node');
        } catch (outerError) {
          try {
      ...
      if (!winptyNative) {
        try {
          winptyNative = require(ptyPath || '../build/Release/pty.node');
        } catch (outerError) {
          try {
